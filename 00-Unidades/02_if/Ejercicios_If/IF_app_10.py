...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        nota = valor_aleatorio
        
        if nota >=6: 
            alert(title="Nota", message=f"Promoción directa, la nota es {nota}")
        else: 
            if nota == 4 or nota == 5:  
                alert(title="Nota", message=f"Aprobado, la nota es {nota}")
            else: 
                alert (title="Nota", message=f"Desaprobado, la nota es {nota}")
